...
3.	Describe the resistance model for uptake of gases into liquid droplets.
...
The atmosphere contains a wide variety of microscopic particulate material, known as aerosol or particulate matter (PM).
Aerosol plays an important role in both the physics and chemistry of the atmosphere -- aerosols are key players in the deleterious impacts of anthropogenic emissions on human health and without aerosols clouds can't form!

Atmospheric aerosols can be liquid, solid or multiphase.
...
Aerosol particles can absorbing and/or scatter radiation and therefore directly influence the radiative balance of the atmosphere.
The intensity of light scattering or absorption is strongly dependent on the size and chemical composition of particles.

The water droplets that comprise clouds always condense on certain pre-existing aerosols called cloud condensation nuclei (CCN).
...
Both processes, light absorption/scattering and could formation, are not very well understood on a global level and are one on the main uncertainties associated with studies predicting the future climate indeed these processes are our largest uncertainty on historic climate change, too.
The uncertainty is mainly due to the large variability in the physical and chemical properties of aerosol particles, as well as their temporal and spatial distributions. Unlike gases like carbon dioxide or methane, we don't know what the levels of aerosols were like in the past. 

```{figure} figures/figure22.0.png
---
name: radiative_forcing
figclass: margin-caption
width: 80%
---
The left hand side shows the impacts on Earth's energy budget cased by changes in gases and aerosols and highlights that the role of aerosols is highly uncertain. The right hand side shows the impact of the changes in energy balance on surface temperature. 
```
...
The maximum in the particle concentration (number) generally lies in the nucleation mode size range. Nucleation mode particles, although numerous, contribute very little to the aerosol mass because they are so small.

The maximum in surface area, which is the most important parameter for light scattering and for acting as sites for heterogeneous reactions, usually lies in the so-called ‘accumulation’ mode. 
...
Particle nucleation is frequently observed over land. Typically, new particles form in more remote areas than very polluted ones but observations show new particle formation occurs also over the ocean and very recently (former [Nat Sci](https://doi.org/10.1038/s41586-024-08196-0) students from Cambridge) have helped show that new particle formation occurs at very high altitudes in the tropics over rain forests.
...
Particles in the accumulation mode typically arise from either: i) the condensation of low volatility vapours onto existing smaller particles, or ii) from coagulation of existing smaller particles.
...
The formation of nitrate aerosols from nitrogen precursor gases has two main natural sources: i) $\mathrm{NO_X}$ from lightning and soils, and ii) $\mathrm{N_2O}$ from bacterial activity in soils and the oceans. Nitrate aerosol is of increasing concern globally owing to a phase out of coal combustion and so reducing levels of sulfate in aerosols. 
...
This is in contrast to $\mathrm{HNO_3}$, which is distributed between the gas and aerosol phases.

The largest source of $\mathrm{SO_2}$ and $\mathrm{NO_x}$ are fossil fuel combustion processes (see lecture 20). 
...
In the presence of $\mathrm{NH_3}$, $\mathrm{HNO_3}$ (one of the major $\mathrm{NO_x}$ reservoir compounds) forms $\mathrm{NH_4NO_3}$.
  ...
  ```{math}
  \\ 
  \mathrm{NH_4NO_{3~(s)} \leftrightarrow NH_{3~(g)} + HNO_{3~(g)}}
  ```
...
values of $v_g$ are typically on the order of $\mathrm{1~ cm~ s^{-1}}$.
...
A large number of metals (exclusively from primary sources) are found in aerosol particles; 
